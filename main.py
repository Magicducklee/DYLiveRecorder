...
def get_flv_url_by_selenium(record_url):
    ...
    try:
        ...
        if urls:
            correct_url = urls[0]
            print(f"✅ Selenium校正成功: {correct_url[:100]}...")
            return correct_url
        else:
            print("⚠️ Selenium未找到合适的URL，将使用原始API地址。")
            return None

    except Exception as e:
        print(f"❌ Selenium校正过程出错: {e}，将使用原始API地址。")
        return None
#==============Selenium代码结束==============
