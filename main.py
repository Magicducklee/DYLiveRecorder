                        ...
                        if port_info['is_live'] is False:
                            ...
                            if real_url:
                                ...
                                for pt_host in overseas_platform_host:
                                    ...
                                    #==============Selenium代码开始==============
                                    print("🔍 DEBUG: Selenium代码块开始执行", flush=True)
                                    # 1. 浏览器配置（无头模式）
                                    options = Options()
                                    ...
                                    time.sleep(15)  # 等待页面加载 JavaScript 内容
                                    ...
                                    # 4. 清洗 + 去重 + 排除 only_audio
                                    if not matches:
                                        print("❌ 没有找到符合要求的 .flv 链接")
                                        sys.exit(1)

                                    urls = []
                                    for url, _ in matches:
                                        if "only_audio" in url:
                                            continue
                                    ...
                                    if not urls:
                                        print("⚠️ 所有匹配的链接都被过滤掉了")
                                        sys.exit(1)

                                    real_url = urls[0]
                                    print(f"[Final Check] ✅即将传递给FFmpeg的 real_url 为: \n{real_url}")
                                    #==============Selenium代码结束==============
